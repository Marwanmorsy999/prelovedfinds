      ...
      <header className="fixed top-0 left-0 right-0 z-50 bg-paper border-b border-hairline transition-all duration-300">
        <div className="mx-auto max-w-7xl flex h-20 items-center justify-between px-4 md:px-8">
          ...
          <div className="flex items-center gap-8 w-1/3">
            <button
              className="md:hidden p-1 text-ink hover:text-concrete transition-colors"
                ...
                return (
                  <Link
                    ...
                    className={`relative text-[12px] font-semibold uppercase tracking-widest transition-colors duration-200 group ${
                      active ? "text-ink" : "text-concrete hover:text-ink"
                    }`}
                  >
                    {item.label}
          ...
          <div className="flex items-center justify-end gap-5 w-1/3" ref={cartRef}>
            {admin && (
              <Link
                to="/admin"
                className="hidden md:block text-[12px] font-semibold uppercase tracking-widest text-concrete hover:text-ink transition-colors"
              >
                Admin
              </Link>
            )}
          ...
          {navLinks.map((item) => {
            ...
            return (
              <Link
                key={item.href}
                to={item.href}
                className="py-3 text-[24px] font-bold uppercase tracking-widest border-b border-surface text-ink transition-colors duration-150"
              >
                {item.label}
              </Link>
            );
